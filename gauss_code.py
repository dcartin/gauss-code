...
Notation:
    ...
    Thus, the sign of the node matches the sign of f(i) from Dowker and
    Thistletwaite.
...
class Symbol:
    ...
    def __init__(self, label = None, chord = None, node_type = None):
    ...
    def getSymbol(self):
        return [self.label, self.chord.label]
    
    #=========================================================================#

    def linkSymbols(self, other):
        """
            Connect two symbols together to represent the same node. Currently,
            this does nothing to the node type.
        """
    ...
    def revSign(self):
        """
            ...
            edges. Currently, this does nothing to the node type.
        """
...
class Graph:
    ...
    def __init__(self):
        ...
        # Create separate variable for the number of vertices, for use in adding
        # node types and constructing the adjacency matrix; leave as 'None' until
    ...
    def assignNodeType(self):
        current = self.head.next
        self.num_vert = 0
        
        while current != None:
            if not current.first.node_type:
                p = random()
                
                if p < 0.25:
                    current.first.node_type = -2
                    current.first.chord.node_type = 2
                    self.num_vert += 1
                    continue
                elif p < 0.5:
                    current.first.node_type = -1
                    current.first.chord.node_type = 1
                    continue
                elif p < 0.75:
                    current.first.node_type = 1
                    current.first.chord.node_type = -1
    ...
    def adjacencyMatrix(self):
        ...
        # Return an empty matrix if the number of vertices is zero
        
        if self.num_vert == 0:
            return np.zeros((0, 0), dtype = int)
        
        # Start at head of list, and create both adjacency matrix for the graph,
        # and dictionary to map between symbol labels (keys) and matrix labels
        # (values)
        
        current = self.head.next
        adj_matrix = np.zeros((self.num_vert, self.num_vert), dtype = np.int8)
        label_dict = {}
        
        # Find the first symbol in the list which is a vertex, and set the
        # value of the first vertex label to this symbol in label_dict
        
        while abs(current.first.node_type) != 2:
            current = current.next
            
        label = 0
        label_dict[abs(current.first.label)] = 0
        current = current.next
        
        # Go through the rest of the edge list, and find all symbols which are
        # vertices. If a new vertex, put an edge in the adjacency matrix;
        # otherwise, move on to next symbol.
        
        while current != None:
            
            # Not a vertex, so move on
            
            if abs(current.first.node_type) != 2:
                current = current.next
                continue

            # If vertex already in label_dict, create appropriate edge
            
            symb = abs(current.first.label)
                
            # print(label, symb, label_dict)
                
            if symb in label_dict:
                symb = label_dict[symb]
                
                adj_matrix[label][symb] += 1
                adj_matrix[symb][label] += 1
                
                label = symb
            else:
                temp_label = max(label_dict.values()) + 1
                label_dict[symb] = temp_label
                
                adj_matrix[label][temp_label] += 1
                adj_matrix[temp_label][label] += 1
                
                label = temp_label
            
            current = current.next
            
        # Last vertex needs to be done, so start at beginning of list to finish
        # the loop around the Gauss code. This initial vertex symbol should
        # already be in label_dict, since we have gone through the list once.
        ...
        last = label_dict[abs(current.first.label)]
        
        adj_matrix[label][last] += 1
        adj_matrix[last][label] += 1
    ...
    def GaussToDT(self, f_flag = True):
        """
            Converts the Gauss code into a DT sequence, as a list of node labels
            in the first entry of a list. If f_flag is True, then the orientation
        ...
        # Go through each edge, and convert Gauss code label to DT label; note
        # that we sort the entries in DT_seq later, so we do not care about
        # matching the correct node labels in the right location in DT_seq
        ...
        # Deal with last label, since this node has not been encountered for
        # the second time
        ...
        # first label. There is no need to sort f_list, since it is indexed
        # by the node labels, which are not changed by the sorting of DT_seq.

        DT_seq = [[node[0], node[1]] if node[0] % 2 == 0 else [node[1], node[0]] for node in DT_seq]
        DT_seq.sort(key = lambda node : node[0])
        
        # Go through Gauss code again, and add node types if they exist; the
        # last symbol can be ignored, since it will always have an odd value of
        # the variable 'label'.
        ...
        while current.next:
            # print(label, current.first.label, current.first.node_type)
            
            # First, check that node type exists. If the label is even, add the
            # node type at the appropriate entry in the DT sequence; otherwise,
            # ignore the node and move on.
            
            if current.first.node_type != None and label % 2 == 0:
                    DT_seq[label // 2] += [current.first.node_type]
    ...
    def renumber(self):
        """
            Go through the Gauss code, and renumber all symbols so that the
            node labels are sequential from 1 to N.
        """
        
        # Initialize node label variable
        
        label = 1
        
        # Create dictionary whose keys are the original symbol labels for the
        # nodes, and the values are the new labels reordered in sequence
        
        label_dict = {}
        
        # Go through list of edges twice: (1) the first time, create a dict
        # between the current node labels and the final ones, then (2) make the
        # actual change
        
        # (1) Identify new labels for each node
        ...
        while current != None:
            ...
            label_dict[symb] = label
                
            label += 1
            current = current.next
            
        # (2) Relabel based on previous runthrough
            
        current = self.head.next
        
        while current != None:
            if current.first.label > 0:
                current.first.label = label_dict[current.first.label]
            elif current.first.label < 0:
                current.first.label = -label_dict[-current.first.label]
